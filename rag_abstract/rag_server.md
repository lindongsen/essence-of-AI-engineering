...
4. 构造“提示词”；
5. 向大模型征求最终答案，该步骤可以调用命令行：`llm_chat "提示词内容"`。你要编写代码去调用命令行，不能省略，也不能写placeholder。

注意：
- 数据检索的步骤中，从步骤2开始要记录时间，输出步骤1之后的每个步骤的执行时间和这些步骤的总耗时。时间只需保留5个小数点。
...
- 单元测试，你必须使用`timeout`去执行测试脚本，避免命令行一直运行而不退出；例如：`timeout 60 uv run pytest tests/unit/test_rag_server.py`。
