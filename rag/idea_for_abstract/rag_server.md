...
- 你要(完整)读取此文件`/林生的奇思妙想/essence-of-AI-engineering/ai_base/rag_base.py`进行参考并使用它，你(必须)原样使用其中的`IterChunks`去生成分片内容，你(不能)改变技术栈，更不能改变(模型)。
- 尽可能将不同功能类别的方法写到不同的文件中，当存在重复任务时，可以检查功能是否在期望中，并且是否可用，从而判断任务是否需要执行。例如：
  - 日志功能`logger.py`
  - 向量计算`embedding.py`
  - 摘要生成`abstract.py`
  - 知识库方法`database.py`
...
这是一个(迭代)事务，下面的步骤都要放在迭代里面进行，达到边读数据边处理的效果，而不是全部读完再处理。举例：`for chunk_text in IterChunk(file): process_chunk(chunk_text)`。
